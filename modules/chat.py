...
def chat():
    try:
        ...
        if user_audio:
            ...
            try:
                response = chat_session.send_message([""" אנא הקשב לקובץ השמע המצורף וענה עליו בקיצור נמרץ בתור תשובה למשתמש אבל אל תגיד למשתמש שקיבלת את ההודעה כקובץ שמע אלא א"כ המשתמש מבקש ואל תגיד משהו בסגנון הזה כגון שמעתי שאמרת "מה קורה" אצלי הכל בסדר אל תגיד אצלי הכל בסדר מה עינינים?.:""", uploaded_audio])
            ...
            return f"read=f-{phone}=,,record,,,no"
